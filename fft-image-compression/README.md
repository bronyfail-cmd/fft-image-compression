...
### Алгоритм (реализован строго по шагам задания)

1. Вычисляется двумерное БПФ изображения (`numpy.fft.fft2`).
2. Амплитуды всех частотных коэффициентов **явно сортируются по убыванию**
   (`numpy.argsort`) — ровно как требует формулировка задания.
3. Оставляются только **N%** коэффициентов с наибольшей амплитудой
   (первые после сортировки), остальные обнуляются.
...
Все результаты (изображения, графики, сжатый файл) сохраняются в папку
`results/` рядом со скриптом — путь определяется автоматически, поэтому
